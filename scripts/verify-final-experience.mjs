...
check('contact-no-overlay', floating.includes('contactVisible') && floating.includes('contactVisible=visibleRatio>=0.15') && floating.includes('const nextVisible=heroPassed&&!contactVisible') && floating.includes('setVisible(nextVisible)') && floating.includes("addEventListener('scroll',schedule,{passive:true})") && floating.includes('[inset-inline-start:18px]') && floating.includes('[inset-inline-end:18px]'), 'Floating PDF/WhatsApp controls use rAF-throttled real scroll geometry, are opposite-sided, and both disappear over the contact/message section.');
...
check('natural-foam-breakup', entrance.includes('site-entry__ocean-foam--one') && entrance.includes('site-entry__ocean-foam--two') && entrance.includes('site-entry__ocean-foam--three') && entrance.includes('site-entry__foam-pockets') && entrance.includes('site-entry__foam-organic') && !entrance.includes('feTurbulence') && !entrance.includes('feDisplacementMap') && css.includes('@keyframes siteEntryFoamOpacityOne') && css.includes('@keyframes siteEntryFoamOpacityTwo') && css.includes('@keyframes siteEntryFoamOpacityThree'), 'Foam breakup uses separately authored crest segments with phase-aligned opacity rhythms and no realtime SVG displacement work.');
